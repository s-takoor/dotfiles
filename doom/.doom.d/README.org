...
** TL;DR
The following is my literate *DOOM EMACS* config file which contains my preferences and modules.
...
** ORG-AUTO-TANGLE
The ~org-auto-tangle~ package automatically tangle org files on save. This is achieved by adding the option ~#+auto_tangle: t~.
...
** VERTICO
Ther ~vertico~ package provides a performant and minimalistic vertical completion IO based on the default completion system.
#+begin_src emacs-lisp :tangle "config.el"
(use-package vertico
  :init
  (vertico-mode)
...
** PACKAGES
To install a package with Doom you must declare in the ~packages.el~ file and run ~doom sync~ on the command line, then restart ~Emacs~ for the changes to take effect -- or use ~M-x doom/reload~.
